...
BEGIN_MESSAGE_MAP(CInstrumentEditorN163Wave, CInstrumentEditPanel)
	ON_COMMAND(IDC_PRESET_SINE, &ThisClass::OnPresetSine)
	ON_COMMAND(IDC_PRESET_TRIANGLE, &ThisClass::OnPresetTriangle)
	ON_COMMAND(IDC_PRESET_SAWTOOTH, &ThisClass::OnPresetSawtooth)
	ON_COMMAND(IDC_PRESET_PULSE_50, &ThisClass::OnPresetPulse50)
	ON_COMMAND(IDC_PRESET_PULSE_25, &ThisClass::OnPresetPulse25)
	ON_MESSAGE(WM_USER_WAVE_CHANGED, &ThisClass::OnWaveChanged)
	ON_BN_CLICKED(IDC_COPY, &ThisClass::OnBnClickedCopy)
	ON_BN_CLICKED(IDC_PASTE, &ThisClass::OnBnClickedPaste)
	ON_CBN_SELCHANGE(IDC_WAVE_SIZE, &ThisClass::OnWaveSizeChange)
	ON_CBN_EDITCHANGE(IDC_WAVE_POS, &ThisClass::OnWavePosChange)
	ON_CBN_SELCHANGE(IDC_WAVE_POS, &ThisClass::OnWavePosSelChange)
//	ON_BN_CLICKED(IDC_POSITION, OnPositionClicked)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_N163_WAVES, OnLvnItemchangedN163Waves)		// // //
	ON_BN_CLICKED(IDC_N163_ADD, &ThisClass::OnBnClickedN163Add)
	ON_BN_CLICKED(IDC_N163_DELETE, &ThisClass::OnBnClickedN163Delete)
END_MESSAGE_MAP()
