...
BEGIN_MESSAGE_MAP(CConfigShortcuts, CPropertyPage)
	ON_NOTIFY(NM_CLICK, IDC_SHORTCUTS, &ThisClass::OnNMClickShortcuts)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_SHORTCUTS, &ThisClass::OnNMClickShortcuts)
	ON_BN_CLICKED(IDC_DEFAULT, &ThisClass::OnBnClickedDefault)
