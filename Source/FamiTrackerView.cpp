...
BEGIN_MESSAGE_MAP(CFamiTrackerView, CView)
	...
	ON_WM_MENUCHAR()
	ON_WM_SYSKEYDOWN()
	ON_COMMAND(ID_EDIT_PASTEMIX, &ThisClass::OnEditPasteMix)
	ON_COMMAND(ID_EDIT_INSTRUMENTMASK, &ThisClass::OnEditInstrumentMask)
	ON_COMMAND(ID_EDIT_VOLUMEMASK, &ThisClass::OnEditVolumeMask)
	ON_COMMAND(ID_EDIT_INTERPOLATE, &ThisClass::OnEditInterpolate)
	ON_COMMAND(ID_EDIT_REVERSE, &ThisClass::OnEditReverse)
	ON_COMMAND(ID_EDIT_REPLACEINSTRUMENT, &ThisClass::OnEditReplaceInstrument)
	ON_COMMAND(ID_TRANSPOSE_DECREASENOTE, &ThisClass::OnTransposeDecreasenote)
	ON_COMMAND(ID_TRANSPOSE_DECREASEOCTAVE, &ThisClass::OnTransposeDecreaseoctave)
	ON_COMMAND(ID_TRANSPOSE_INCREASENOTE, &ThisClass::OnTransposeIncreasenote)
	ON_COMMAND(ID_TRANSPOSE_INCREASEOCTAVE, &ThisClass::OnTransposeIncreaseoctave)
	ON_COMMAND(ID_DECREASEVALUES, &ThisClass::OnDecreaseValues)
	ON_COMMAND(ID_INCREASEVALUES, &ThisClass::OnIncreaseValues)
	ON_COMMAND(ID_TRACKER_PLAYROW, &ThisClass::OnTrackerPlayrow)
	ON_COMMAND(ID_TRACKER_EDIT, &ThisClass::OnTrackerEdit)
	ON_COMMAND(ID_TRACKER_TOGGLECHANNEL, &ThisClass::OnTrackerToggleChannel)
	ON_COMMAND(ID_TRACKER_SOLOCHANNEL, &ThisClass::OnTrackerSoloChannel)
	ON_COMMAND(ID_CMD_INCREASESTEPSIZE, &ThisClass::OnIncreaseStepSize)
	ON_COMMAND(ID_CMD_DECREASESTEPSIZE, &ThisClass::OnDecreaseStepSize)
	ON_COMMAND(ID_CMD_STEP_UP, &ThisClass::OnOneStepUp)
	ON_COMMAND(ID_CMD_STEP_DOWN, &ThisClass::OnOneStepDown)	
	ON_COMMAND(ID_POPUP_TOGGLECHANNEL, &ThisClass::OnTrackerToggleChannel)
	ON_COMMAND(ID_POPUP_SOLOCHANNEL, &ThisClass::OnTrackerSoloChannel)
	ON_COMMAND(ID_POPUP_UNMUTEALLCHANNELS, &ThisClass::OnTrackerUnmuteAllChannels)
//	ON_UPDATE_COMMAND_UI(ID_EDIT_CUT, OnUpdateEditCut)
//	ON_UPDATE_COMMAND_UI(ID_EDIT_DELETE, OnUpdateEditDelete)
	ON_UPDATE_COMMAND_UI(ID_EDIT_INSTRUMENTMASK, &ThisClass::OnUpdateEditInstrumentMask)
	ON_UPDATE_COMMAND_UI(ID_EDIT_VOLUMEMASK, &ThisClass::OnUpdateEditVolumeMask)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_EDIT, &ThisClass::OnUpdateTrackerEdit)
	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTEMIX, &ThisClass::OnUpdateEditPaste)
	ON_WM_NCMOUSEMOVE()
	ON_COMMAND(ID_BLOCK_START, &ThisClass::OnBlockStart)	
	ON_COMMAND(ID_BLOCK_END, &ThisClass::OnBlockEnd)
	ON_COMMAND(ID_POPUP_PICKUPROW, &ThisClass::OnPickupRow)
	ON_MESSAGE(WM_USER_MIDI_EVENT, &ThisClass::OnUserMidiEvent)
	ON_MESSAGE(WM_USER_PLAYER, &ThisClass::OnUserPlayerEvent)
	ON_MESSAGE(WM_USER_NOTE_EVENT, &ThisClass::OnUserNoteEvent)
	ON_WM_CLOSE()
	ON_WM_DESTROY()
	// // //
	ON_MESSAGE(WM_USER_DUMP_INST, &ThisClass::OnUserDumpInst)
	ON_COMMAND(ID_MODULE_DETUNE, &ThisClass::OnTrackerDetune)
	ON_UPDATE_COMMAND_UI(ID_FIND_NEXT, &ThisClass::OnUpdateFind)
	ON_UPDATE_COMMAND_UI(ID_FIND_PREVIOUS, &ThisClass::OnUpdateFind)
	ON_COMMAND(ID_DECREASEVALUESCOARSE, &ThisClass::OnCoarseDecreaseValues)
	ON_COMMAND(ID_INCREASEVALUESCOARSE, &ThisClass::OnCoarseIncreaseValues)
//	ON_COMMAND(ID_EDIT_PASTEOVERWRITE, OnEditPasteOverwrite)
	ON_COMMAND(ID_EDIT_PASTEINSERT, &ThisClass::OnEditPasteInsert)
//	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTEOVERWRITE, OnUpdateEditPaste)
	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTEINSERT, &ThisClass::OnUpdateEditPaste)
	ON_COMMAND(ID_PASTESPECIAL_CURSOR, &ThisClass::OnEditPasteSpecialCursor)
	ON_COMMAND(ID_PASTESPECIAL_SELECTION, &ThisClass::OnEditPasteSpecialSelection)
	ON_COMMAND(ID_PASTESPECIAL_FILL, &ThisClass::OnEditPasteSpecialFill)
	ON_UPDATE_COMMAND_UI(ID_PASTESPECIAL_CURSOR, &ThisClass::OnUpdatePasteSpecial)
	ON_UPDATE_COMMAND_UI(ID_PASTESPECIAL_SELECTION, &ThisClass::OnUpdatePasteSpecial)
	ON_UPDATE_COMMAND_UI(ID_PASTESPECIAL_FILL, &ThisClass::OnUpdatePasteSpecial)
	ON_COMMAND(ID_BOOKMARKS_TOGGLE, &ThisClass::OnBookmarksToggle)
	ON_COMMAND(ID_BOOKMARKS_NEXT, &ThisClass::OnBookmarksNext)
	ON_COMMAND(ID_BOOKMARKS_PREVIOUS, &ThisClass::OnBookmarksPrevious)
	ON_COMMAND(ID_EDIT_SPLITKEYBOARD, &ThisClass::OnEditSplitKeyboard)
	ON_COMMAND(ID_TRACKER_TOGGLECHIP, &ThisClass::OnTrackerToggleChip)
	ON_COMMAND(ID_TRACKER_SOLOCHIP, &ThisClass::OnTrackerSoloChip)
	ON_COMMAND(ID_TRACKER_RECORDTOINST, &ThisClass::OnTrackerRecordToInst)
	ON_COMMAND(ID_TRACKER_RECORDERSETTINGS, &ThisClass::OnTrackerRecorderSettings)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_RECORDTOINST, &ThisClass::OnUpdateDisableWhilePlaying)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_RECORDERSETTINGS, &ThisClass::OnUpdateDisableWhilePlaying)
	ON_COMMAND(ID_RECALL_CHANNEL_STATE, &ThisClass::OnRecallChannelState)
