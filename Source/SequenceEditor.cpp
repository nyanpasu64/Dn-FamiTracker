...
BEGIN_MESSAGE_MAP(CSequenceEditor, CWnd)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_MESSAGE(WM_SIZE_CHANGE, &ThisClass::OnSizeChange)
	ON_MESSAGE(WM_CURSOR_CHANGE, &ThisClass::OnCursorChange)
	ON_MESSAGE(WM_SEQUENCE_CHANGED, &ThisClass::OnSequenceChanged)
	ON_MESSAGE(WM_SETTING_CHANGED, &ThisClass::OnSettingChanged)
END_MESSAGE_MAP()
