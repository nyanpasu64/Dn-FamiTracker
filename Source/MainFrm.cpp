...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_WM_DESTROY()
	ON_WM_COPYDATA()
	ON_COMMAND(ID_FILE_GENERALSETTINGS, &ThisClass::OnFileGeneralsettings)
	ON_COMMAND(ID_FILE_IMPORTTEXT, &ThisClass::OnFileImportText)
	ON_COMMAND(ID_FILE_EXPORTTEXT, &ThisClass::OnFileExportText)
	ON_COMMAND(ID_FILE_CREATE_NSF, &ThisClass::OnCreateNSF)
	ON_COMMAND(ID_FILE_CREATEWAV, &ThisClass::OnCreateWAV)
	...
	ON_COMMAND(ID_EDIT_ENABLEMIDI, &ThisClass::OnEditEnableMIDI)
	ON_COMMAND(ID_EDIT_EXPANDPATTERNS, &ThisClass::OnEditExpandpatterns)
	ON_COMMAND(ID_EDIT_SHRINKPATTERNS, &ThisClass::OnEditShrinkpatterns)
	ON_COMMAND(ID_EDIT_CLEARPATTERNS, &ThisClass::OnEditClearPatterns)
	ON_COMMAND(ID_CLEANUP_REMOVEUNUSEDINSTRUMENTS, &ThisClass::OnEditRemoveUnusedInstruments)
	ON_COMMAND(ID_CLEANUP_REMOVEUNUSEDPATTERNS, &ThisClass::OnEditRemoveUnusedPatterns)
	ON_COMMAND(ID_CLEANUP_MERGEDUPLICATEDPATTERNS, &ThisClass::OnEditMergeDuplicatedPatterns)
	ON_COMMAND(ID_INSTRUMENT_NEW, &ThisClass::OnAddInstrument)
	ON_COMMAND(ID_INSTRUMENT_REMOVE, &ThisClass::OnRemoveInstrument)
	ON_COMMAND(ID_INSTRUMENT_CLONE, &ThisClass::OnCloneInstrument)
	ON_COMMAND(ID_INSTRUMENT_DEEPCLONE, &ThisClass::OnDeepCloneInstrument)
	ON_COMMAND(ID_INSTRUMENT_SAVE, &ThisClass::OnSaveInstrument)
	ON_COMMAND(ID_INSTRUMENT_LOAD, &ThisClass::OnLoadInstrument)
	ON_COMMAND(ID_INSTRUMENT_EDIT, &ThisClass::OnEditInstrument)
	ON_COMMAND(ID_INSTRUMENT_ADD_2A03, &ThisClass::OnAddInstrument2A03)
	ON_COMMAND(ID_INSTRUMENT_ADD_VRC6, &ThisClass::OnAddInstrumentVRC6)
	ON_COMMAND(ID_INSTRUMENT_ADD_VRC7, &ThisClass::OnAddInstrumentVRC7)
	ON_COMMAND(ID_INSTRUMENT_ADD_FDS, &ThisClass::OnAddInstrumentFDS)
	ON_COMMAND(ID_INSTRUMENT_ADD_MMC5, &ThisClass::OnAddInstrumentMMC5)
	ON_COMMAND(ID_INSTRUMENT_ADD_N163, &ThisClass::OnAddInstrumentN163)
	ON_COMMAND(ID_INSTRUMENT_ADD_S5B, &ThisClass::OnAddInstrumentS5B)
	ON_COMMAND(ID_MODULE_MODULEPROPERTIES, &ThisClass::OnModuleModuleproperties)
	ON_COMMAND(ID_MODULE_CHANNELS, &ThisClass::OnModuleChannels)
	ON_COMMAND(ID_MODULE_COMMENTS, &ThisClass::OnModuleComments)
	ON_COMMAND(ID_MODULE_INSERTFRAME, &ThisClass::OnModuleInsertFrame)
	ON_COMMAND(ID_MODULE_REMOVEFRAME, &ThisClass::OnModuleRemoveFrame)
	ON_COMMAND(ID_MODULE_DUPLICATEFRAME, &ThisClass::OnModuleDuplicateFrame)
	ON_COMMAND(ID_MODULE_DUPLICATEFRAMEPATTERNS, &ThisClass::OnModuleDuplicateFramePatterns)
	ON_COMMAND(ID_MODULE_MOVEFRAMEDOWN, &ThisClass::OnModuleMoveframedown)
	ON_COMMAND(ID_MODULE_MOVEFRAMEUP, &ThisClass::OnModuleMoveframeup)
	ON_COMMAND(ID_TRACKER_PLAY, &ThisClass::OnTrackerPlay)
	ON_COMMAND(ID_TRACKER_PLAY_START, &ThisClass::OnTrackerPlayStart)
	ON_COMMAND(ID_TRACKER_PLAY_CURSOR, &ThisClass::OnTrackerPlayCursor)
	ON_COMMAND(ID_TRACKER_STOP, &ThisClass::OnTrackerStop)
	ON_COMMAND(ID_TRACKER_TOGGLE_PLAY, &ThisClass::OnTrackerTogglePlay)
	ON_COMMAND(ID_TRACKER_PLAYPATTERN, &ThisClass::OnTrackerPlaypattern)
	ON_COMMAND(ID_TRACKER_KILLSOUND, &ThisClass::OnTrackerKillsound)
	ON_COMMAND(ID_TRACKER_SWITCHTOTRACKINSTRUMENT, &ThisClass::OnTrackerSwitchToInstrument)
	// // //
	ON_COMMAND(ID_TRACKER_DISPLAYREGISTERSTATE, &ThisClass::OnTrackerDisplayRegisterState)
	ON_COMMAND(ID_VIEW_CONTROLPANEL, &ThisClass::OnViewControlpanel)
	ON_COMMAND(ID_HELP, &ThisClass::CFrameWnd::OnHelp)
	ON_COMMAND(ID_HELP_FINDER, &ThisClass::CFrameWnd::OnHelpFinder)
	ON_COMMAND(ID_HELP_PERFORMANCE, &ThisClass::OnHelpPerformance)
	ON_COMMAND(ID_HELP_EFFECTTABLE, &CMainFrame::OnHelpEffecttable)
	ON_COMMAND(ID_HELP_FAQ, &CMainFrame::OnHelpFAQ)
	ON_COMMAND(ID_DEFAULT_HELP, &ThisClass::CFrameWnd::OnHelpFinder)
	ON_COMMAND(ID_CONTEXT_HELP, &ThisClass::CFrameWnd::OnContextHelp)
	ON_COMMAND(ID_FRAMEEDITOR_TOP, &ThisClass::OnFrameeditorTop)
	ON_COMMAND(ID_FRAMEEDITOR_LEFT, &ThisClass::OnFrameeditorLeft)
	ON_COMMAND(ID_NEXT_FRAME, &ThisClass::OnNextFrame)
	ON_COMMAND(ID_PREV_FRAME, &ThisClass::OnPrevFrame)
	ON_COMMAND(IDC_KEYREPEAT, &ThisClass::OnKeyRepeat)
	ON_COMMAND(ID_NEXT_SONG, &ThisClass::OnNextSong)
	ON_COMMAND(ID_PREV_SONG, &ThisClass::OnPrevSong)
	ON_COMMAND(IDC_FOLLOW_TOGGLE, &ThisClass::OnToggleFollow)
	ON_COMMAND(ID_FOCUS_PATTERN_EDITOR, &ThisClass::OnSelectPatternEditor)
	ON_COMMAND(ID_FOCUS_FRAME_EDITOR, &ThisClass::OnSelectFrameEditor)
	ON_COMMAND(ID_CMD_NEXT_INSTRUMENT, &ThisClass::OnNextInstrument)
	ON_COMMAND(ID_CMD_PREV_INSTRUMENT, &ThisClass::OnPrevInstrument)
	ON_COMMAND(ID_TOGGLE_SPEED, &ThisClass::OnToggleSpeed)
	ON_COMMAND(ID_DECAY_FAST, &ThisClass::OnDecayFast)
	ON_COMMAND(ID_DECAY_SLOW, &ThisClass::OnDecaySlow)
	ON_BN_CLICKED(IDC_FRAME_INC, &ThisClass::OnBnClickedIncFrame)
	ON_BN_CLICKED(IDC_FRAME_DEC, &ThisClass::OnBnClickedDecFrame)
	ON_BN_CLICKED(IDC_FOLLOW, &ThisClass::OnClickedFollow)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPEED_SPIN, &ThisClass::OnDeltaposSpeedSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_TEMPO_SPIN, &ThisClass::OnDeltaposTempoSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_ROWS_SPIN, &ThisClass::OnDeltaposRowsSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_FRAME_SPIN, &ThisClass::OnDeltaposFrameSpin)
	ON_NOTIFY(UDN_DELTAPOS, IDC_KEYSTEP_SPIN, &ThisClass::OnDeltaposKeyStepSpin)
	ON_EN_CHANGE(IDC_INSTNAME, &ThisClass::OnInstNameChange)
	ON_EN_CHANGE(IDC_KEYSTEP, &ThisClass::OnEnKeyStepChange)
	ON_EN_CHANGE(IDC_SONG_NAME, &ThisClass::OnEnSongNameChange)
	ON_EN_CHANGE(IDC_SONG_ARTIST, &ThisClass::OnEnSongArtistChange)
	ON_EN_CHANGE(IDC_SONG_COPYRIGHT, &ThisClass::OnEnSongCopyrightChange)
	ON_EN_SETFOCUS(IDC_KEYREPEAT, &ThisClass::OnRemoveFocus)
	...
	ON_UPDATE_COMMAND_UI(ID_EDIT_DELETE, &ThisClass::OnUpdateEditDelete)
	ON_UPDATE_COMMAND_UI(ID_EDIT_ENABLEMIDI, &ThisClass::OnUpdateEditEnablemidi)
	ON_UPDATE_COMMAND_UI(ID_MODULE_INSERTFRAME, &ThisClass::OnUpdateInsertFrame)
	ON_UPDATE_COMMAND_UI(ID_MODULE_REMOVEFRAME, &ThisClass::OnUpdateRemoveFrame)
	ON_UPDATE_COMMAND_UI(ID_MODULE_DUPLICATEFRAME, &ThisClass::OnUpdateDuplicateFrame)
	ON_UPDATE_COMMAND_UI(ID_MODULE_MOVEFRAMEDOWN, &ThisClass::OnUpdateModuleMoveframedown)
	ON_UPDATE_COMMAND_UI(ID_MODULE_MOVEFRAMEUP, &ThisClass::OnUpdateModuleMoveframeup)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_NEW, &ThisClass::OnUpdateInstrumentNew)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_REMOVE, &ThisClass::OnUpdateInstrumentRemove)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_CLONE, &ThisClass::OnUpdateInstrumentClone)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_DEEPCLONE, &ThisClass::OnUpdateInstrumentDeepClone)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_EDIT, &ThisClass::OnUpdateInstrumentEdit)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_LOAD, &ThisClass::OnUpdateInstrumentLoad)
	ON_UPDATE_COMMAND_UI(ID_INSTRUMENT_SAVE, &ThisClass::OnUpdateInstrumentSave)
	ON_UPDATE_COMMAND_UI(ID_INDICATOR_INSTRUMENT, &ThisClass::OnUpdateSBInstrument)
	ON_UPDATE_COMMAND_UI(ID_INDICATOR_OCTAVE, &ThisClass::OnUpdateSBOctave)
	ON_UPDATE_COMMAND_UI(ID_INDICATOR_RATE, &ThisClass::OnUpdateSBFrequency)
	ON_UPDATE_COMMAND_UI(ID_INDICATOR_TEMPO, &ThisClass::OnUpdateSBTempo)
	ON_UPDATE_COMMAND_UI(ID_INDICATOR_CHIP, &ThisClass::OnUpdateSBChip)
	ON_UPDATE_COMMAND_UI(IDC_KEYSTEP, &ThisClass::OnUpdateKeyStepEdit)
	ON_UPDATE_COMMAND_UI(IDC_KEYREPEAT, &ThisClass::OnUpdateKeyRepeat)
	ON_UPDATE_COMMAND_UI(IDC_SPEED, &ThisClass::OnUpdateSpeedEdit)
	ON_UPDATE_COMMAND_UI(IDC_TEMPO, &ThisClass::OnUpdateTempoEdit)
	ON_UPDATE_COMMAND_UI(IDC_ROWS, &ThisClass::OnUpdateRowsEdit)
	ON_UPDATE_COMMAND_UI(IDC_FRAMES, &ThisClass::OnUpdateFramesEdit)
	ON_UPDATE_COMMAND_UI(ID_NEXT_SONG, &ThisClass::OnUpdateNextSong)
	ON_UPDATE_COMMAND_UI(ID_PREV_SONG, &ThisClass::OnUpdatePrevSong)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_SWITCHTOTRACKINSTRUMENT, &ThisClass::OnUpdateTrackerSwitchToInstrument)
	ON_UPDATE_COMMAND_UI(ID_VIEW_CONTROLPANEL, &ThisClass::OnUpdateViewControlpanel)
	ON_UPDATE_COMMAND_UI(ID_EDIT_EXPANDPATTERNS, &ThisClass::OnUpdateSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_EDIT_SHRINKPATTERNS, &ThisClass::OnUpdateSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_FRAMEEDITOR_TOP, &ThisClass::OnUpdateFrameeditorTop)
	ON_UPDATE_COMMAND_UI(ID_FRAMEEDITOR_LEFT, &ThisClass::OnUpdateFrameeditorLeft)
	ON_CBN_SELCHANGE(IDC_SUBTUNE, &ThisClass::OnCbnSelchangeSong)
	ON_CBN_SELCHANGE(IDC_OCTAVE, &ThisClass::OnCbnSelchangeOctave)
	ON_MESSAGE(WM_USER_DISPLAY_MESSAGE_STRING, &ThisClass::OnDisplayMessageString)
	ON_MESSAGE(WM_USER_DISPLAY_MESSAGE_ID, &ThisClass::OnDisplayMessageID)
	ON_COMMAND(ID_VIEW_TOOLBAR, &CMainFrame::OnViewToolbar)

	// // //
	ON_BN_CLICKED(IDC_BUTTON_GROOVE, &ThisClass::OnToggleGroove)
	ON_BN_CLICKED(IDC_BUTTON_FIXTEMPO, &ThisClass::OnToggleFixTempo)
	ON_BN_CLICKED(IDC_CHECK_COMPACT, &ThisClass::OnClickedCompact)
	ON_COMMAND(ID_CMD_INST_NUM, &ThisClass::OnTypeInstrumentNumber)
	ON_COMMAND(IDC_COMPACT_TOGGLE, &ThisClass::OnToggleCompact)
	ON_UPDATE_COMMAND_UI(IDC_HIGHLIGHT1, &ThisClass::OnUpdateHighlight1)
	ON_UPDATE_COMMAND_UI(IDC_HIGHLIGHT2, &ThisClass::OnUpdateHighlight2)
	ON_NOTIFY(UDN_DELTAPOS, IDC_HIGHLIGHTSPIN1, &ThisClass::OnDeltaposHighlightSpin1)
	ON_NOTIFY(UDN_DELTAPOS, IDC_HIGHLIGHTSPIN2, &ThisClass::OnDeltaposHighlightSpin2)
	ON_COMMAND(ID_FILE_EXPORTROWS, &ThisClass::OnFileExportRows)
	ON_COMMAND(ID_COPYAS_TEXT, &ThisClass::OnEditCopyAsText)
	ON_COMMAND(ID_COPYAS_VOLUMESEQUENCE, &ThisClass::OnEditCopyAsVolumeSequence)
	ON_COMMAND(ID_COPYAS_PPMCK, &ThisClass::OnEditCopyAsPPMCK)
	ON_COMMAND(ID_EDIT_PASTEOVERWRITE, &ThisClass::OnEditPasteOverwrite)
	ON_COMMAND(ID_SELECT_NONE, &ThisClass::OnEditSelectnone)
	ON_COMMAND(ID_SELECT_ROW, &ThisClass::OnEditSelectrow)
	ON_COMMAND(ID_SELECT_COLUMN, &ThisClass::OnEditSelectcolumn)
	ON_COMMAND(ID_SELECT_PATTERN, &ThisClass::OnEditSelectpattern)
	ON_COMMAND(ID_SELECT_FRAME, &ThisClass::OnEditSelectframe)
	ON_COMMAND(ID_SELECT_CHANNEL, &ThisClass::OnEditSelectchannel)
	ON_COMMAND(ID_SELECT_TRACK, &ThisClass::OnEditSelecttrack)
	ON_COMMAND(ID_SELECT_OTHER, &ThisClass::OnEditSelectother)
	ON_COMMAND(ID_EDIT_FIND_TOGGLE, &ThisClass::OnEditFindToggle)
	ON_COMMAND(ID_FIND_NEXT, &ThisClass::OnFindNext)
	ON_COMMAND(ID_FIND_PREVIOUS, &ThisClass::OnFindPrevious)
	ON_COMMAND(ID_EDIT_GOTO, &ThisClass::OnEditGoto)
	ON_COMMAND(ID_EDIT_SWAPCHANNELS, &ThisClass::OnEditSwapChannels)
	ON_COMMAND(ID_EDIT_STRETCHPATTERNS, &ThisClass::OnEditStretchpatterns)
	ON_COMMAND(ID_TRANSPOSE_CUSTOM, &ThisClass::OnEditTransposeCustom)
	ON_COMMAND(ID_CLEANUP_REMOVEUNUSEDDPCMSAMPLES, &ThisClass::OnEditRemoveUnusedSamples)
	ON_COMMAND(ID_CLEANUP_POPULATEUNIQUEPATTERNS, &ThisClass::OnEditPopulateUniquePatterns)
	ON_COMMAND(ID_MODULE_DUPLICATECURRENTPATTERN, &ThisClass::OnModuleDuplicateCurrentPattern)
	ON_COMMAND(ID_MODULE_GROOVE, &ThisClass::OnModuleGrooveSettings)
	ON_COMMAND(ID_MODULE_BOOKMARK, &ThisClass::OnModuleBookmarkSettings)
	ON_COMMAND(ID_MODULE_ESTIMATESONGLENGTH, &ThisClass::OnModuleEstimateSongLength)
	...
	ON_COMMAND(ID_TOGGLE_MULTIPLEXER, &ThisClass::OnToggleMultiplexer)
	ON_UPDATE_COMMAND_UI(IDC_FOLLOW_TOGGLE, &ThisClass::OnUpdateToggleFollow)
	ON_UPDATE_COMMAND_UI(IDC_COMPACT_TOGGLE, &ThisClass::OnUpdateToggleCompact)
	ON_UPDATE_COMMAND_UI(IDC_BUTTON_FIXTEMPO, &ThisClass::OnUpdateToggleFixTempo)
	ON_UPDATE_COMMAND_UI(IDC_BUTTON_GROOVE, &ThisClass::OnUpdateGrooveEdit)
	ON_UPDATE_COMMAND_UI(ID_COPYAS_TEXT, &ThisClass::OnUpdateEditCopySpecial)
	ON_UPDATE_COMMAND_UI(ID_COPYAS_VOLUMESEQUENCE, &ThisClass::OnUpdateEditCopySpecial)
	ON_UPDATE_COMMAND_UI(ID_COPYAS_PPMCK, &ThisClass::OnUpdateEditCopySpecial)
	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTEOVERWRITE, &ThisClass::OnUpdateEditPasteOverwrite)
	ON_UPDATE_COMMAND_UI(ID_SELECT_ROW, &ThisClass::OnUpdatePatternEditorSelected)
	ON_UPDATE_COMMAND_UI(ID_SELECT_COLUMN, &ThisClass::OnUpdatePatternEditorSelected)
	ON_UPDATE_COMMAND_UI(ID_SELECT_CHANNEL, &ThisClass::OnUpdateSelectMultiFrame)
	ON_UPDATE_COMMAND_UI(ID_SELECT_TRACK, &ThisClass::OnUpdateSelectMultiFrame)
//	ON_UPDATE_COMMAND_UI(ID_SELECT_OTHER, OnUpdateCurrentSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_EDIT_FIND_TOGGLE, &ThisClass::OnUpdateEditFindToggle)
	ON_UPDATE_COMMAND_UI(ID_EDIT_INTERPOLATE, &ThisClass::OnUpdateSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_EDIT_REVERSE, &ThisClass::OnUpdateSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_EDIT_REPLACEINSTRUMENT, &ThisClass::OnUpdateSelectionEnabled)
	ON_UPDATE_COMMAND_UI(ID_EDIT_STRETCHPATTERNS, &ThisClass::OnUpdateSelectionEnabled)
	...
	ON_UPDATE_COMMAND_UI(ID_TRACKER_DISPLAYREGISTERSTATE, &ThisClass::OnUpdateDisplayRegisterState)
	...
	ON_COMMAND(ID_KRAID2, &ThisClass::OnEasterEggKraid2)
	ON_COMMAND(ID_KRAID3, &ThisClass::OnEasterEggKraid3)
	ON_COMMAND(ID_KRAID4, &ThisClass::OnEasterEggKraid4)
	ON_COMMAND(ID_KRAID5, &ThisClass::OnEasterEggKraid5)
	// // // From CFamiTrackerView
	ON_COMMAND(ID_CMD_OCTAVE_NEXT, &ThisClass::OnNextOctave)
	ON_COMMAND(ID_CMD_OCTAVE_PREVIOUS, &ThisClass::OnPreviousOctave)
	ON_COMMAND(ID_TRACKER_PAL, &ThisClass::OnTrackerPal)
	ON_COMMAND(ID_TRACKER_NTSC, &ThisClass::OnTrackerNtsc)
	ON_COMMAND(ID_SPEED_DEFAULT, &ThisClass::OnSpeedDefault)
	ON_COMMAND(ID_SPEED_CUSTOM, &ThisClass::OnSpeedCustom)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_PAL, &ThisClass::OnUpdateTrackerPal)
	ON_UPDATE_COMMAND_UI(ID_TRACKER_NTSC, &ThisClass::OnUpdateTrackerNtsc)
	ON_UPDATE_COMMAND_UI(ID_SPEED_DEFAULT, &ThisClass::OnUpdateSpeedDefault)
	ON_UPDATE_COMMAND_UI(ID_SPEED_CUSTOM, &ThisClass::OnUpdateSpeedCustom)
	END_MESSAGE_MAP()
