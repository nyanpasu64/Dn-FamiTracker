...
BEGIN_MESSAGE_MAP(CInstrumentEditorFDS, CInstrumentEditPanel)
	...
	ON_COMMAND(IDC_MOD_PRESET_FLAT, &ThisClass::OnModPresetFlat)
	ON_COMMAND(IDC_MOD_PRESET_SINE, &ThisClass::OnModPresetSine)
	ON_WM_VSCROLL()
	ON_EN_CHANGE(IDC_MOD_RATE, &ThisClass::OnModRateChange)
	ON_EN_CHANGE(IDC_MOD_DEPTH, &ThisClass::OnModDepthChange)
	ON_EN_CHANGE(IDC_MOD_DELAY, &ThisClass::OnModDelayChange)
...
//	ON_BN_CLICKED(IDC_ENABLE_FM, &CInstrumentEditorFDS::OnBnClickedEnableFm)
	ON_MESSAGE(WM_USER + 1, &ThisClass::OnModChanged)
END_MESSAGE_MAP()
